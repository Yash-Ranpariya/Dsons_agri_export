        ...
        <AnimatePresence>
            ...
            >
                <motion.div
                    ...
                    className="bg-white dark:bg-dark-surface rounded-3xl overflow-hidden w-full max-w-5xl shadow-2xl flex flex-col md:flex-row my-8 relative border border-white/20 dark:border-white/10"
                    onClick={(e) => e.stopPropagation()}
                >
                    {/* Close Button */}
                    <button
                        onClick={onClose}
                        className="absolute top-4 right-4 z-10 bg-white/50 dark:bg-black/50 hover:bg-white dark:hover:bg-primary text-gray-800 dark:text-gray-200 p-2 rounded-full backdrop-blur-md shadow-sm transition-colors"
                    >
                        <X size={24} />
                    </button>

                    {/* Left Column: Image & Visual Packet Mockup */}
                    <div className="w-full md:w-2/5 bg-gray-50 dark:bg-dark-bg flex flex-col">
                        ...
                        {/* Packaging Mockup */}
                        <div className="flex-1 p-8 flex flex-col items-center justify-center bg-[#fdf8f5] dark:bg-dark-surface overflow-hidden relative">
                    ...
                    <div className="w-full md:w-3/5 p-8 md:p-12 flex flex-col relative z-10">
                        <span className="text-primary dark:text-accent font-bold tracking-widest uppercase text-sm mb-2">{product.category || 'Premium Spice'}</span>
                        <h2 className="text-4xl md:text-5xl font-serif font-extrabold text-gray-900 dark:text-white mb-6">{product.name}</h2>
                        <p className="text-gray-600 dark:text-gray-300 text-lg leading-relaxed mb-8 border-b border-gray-100 dark:border-gray-800 pb-8">
                            {product.desc}
                        </p>

                        {/* Informational Grid */}
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
                            <div className="bg-gray-50 dark:bg-black/40 rounded-xl p-4 border border-gray-100 dark:border-gray-800 backdrop-blur-sm">
                                <MapPin className="text-primary dark:text-accent mb-3" size={24} />
                                <h4 className="font-bold text-gray-900 dark:text-white text-sm uppercase tracking-wide mb-1">Origin</h4>
                                <p className="text-gray-600 dark:text-gray-400 text-sm">{product.origin}</p>
                            </div>
                            <div className="bg-gray-50 dark:bg-black/40 rounded-xl p-4 border border-gray-100 dark:border-gray-800 backdrop-blur-sm">
                                <Calendar className="text-primary dark:text-accent mb-3" size={24} />
                                <h4 className="font-bold text-gray-900 dark:text-white text-sm uppercase tracking-wide mb-1">Harvest time</h4>
                                <p className="text-gray-600 dark:text-gray-400 text-sm">{product.harvestSeason}</p>
                            </div>
                            <div className="bg-gray-50 dark:bg-black/40 rounded-xl p-4 border border-gray-100 dark:border-gray-800 backdrop-blur-sm">
                                <Clock className="text-primary dark:text-accent mb-3" size={24} />
                                <h4 className="font-bold text-gray-900 dark:text-white text-sm uppercase tracking-wide mb-1">Shelf Life</h4>
                                <p className="text-gray-600 dark:text-gray-400 text-sm">{product.shelfLife}</p>
                            </div>
                        </div>

                        {/* Weight Selector */}
                        <div className="mt-auto">
                            <h4 className="font-bold text-gray-900 dark:text-white mb-4 inline-flex items-center">
                                ...
                                {weights.map((w) => (
                                    <button
                                        ...
                                        className={`px-5 py-2.5 rounded-lg border-2 font-bold text-sm transition-all ${selectedWeight === w
                                            ? 'border-primary bg-primary text-white shadow-[0_0_15px_rgba(139,26,32,0.4)]'
                                            : 'border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-primary dark:hover:border-accent hover:text-primary dark:hover:text-accent'
                                            }`}
                                    >
                                        {w}
                                    </button>
                            ...
                            <a href={`mailto:export@ranpariyassons.com?subject=Inquiry for ${product.name} (${selectedWeight})`} className="w-full md:w-auto inline-block text-center bg-gray-900 dark:bg-primary hover:bg-gray-800 dark:hover:bg-primary/80 text-white font-bold py-4 px-10 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.15)] dark:shadow-[0_4px_20px_rgba(139,26,32,0.4)] hover:-translate-y-1 transition-all duration-300">
                                Request Bulk Quote
                            </a>
                        </div>
                    </div>
